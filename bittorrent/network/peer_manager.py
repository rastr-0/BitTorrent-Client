...
from bittorrent.network.peer_connection import Peer


class PeerManager(Thread):
    def __init__(self, piece_manager, info_hash: bytes, number_of_pieces: int = -1):
    ...
    def connect_to_peers(self, peers_list: list) -> None:
        """Establish TCP connections and perform handshakes with peers."""
        print("The process of establishing connections and doing a handshakes started...")
        for peer in peers_list:
            peer_obj = Peer(
                self.piece_manager, self.number_of_pieces,
                peer['ip'], self.info_hash, peer['port'],
            )
            if peer_obj.connect():
                if peer_obj.handshake():
    ...
    def disconnect_peer(self, peer):
        if peer in self.connected_peers:
            try:
                ...
                logging.error(f"Unable to close connection with peer: {peer.ip_address}:{peer.port}")
